...
public class DemoBatalla {
    public static void main(String [] args){
        ...
        for (int i = 0; i < misNaves.length; i++) {
            ...
            System.out.println("Nuevo orden de la flota:");
            System.out.println(desordenarNaves(misNaves));
        }
    }
    ...
    public static Nave [] desordenarNaves(Nave [] flota){
        ...
        int posicion;
        for(int i = 0; i < flota.length; i++){
            posicion = (int)(Math.random() * flota.length);
            //Si la posicion NO se encuentra ocupada
            for(int j = 0; j < nuevaFlota.length; j++){
                if (! flota[i].getNombre().equals(nuevaFlota[j].getNombre())){
                    nuevaFlota[posicion] = flota[i];
                }
            }
        }
        return nuevaFlota;
